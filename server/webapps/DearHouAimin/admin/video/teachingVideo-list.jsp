...
<body>
	<nav class="breadcrumb">
		...
		教学录像 <span class="c-gray en">&gt;</span> 教学录像 <a
	...
	<div class="page-container">
		<form action="/ResourceServlet.do" class="text-c" method="post">
			<input type="hidden" name="resType" value="2" /> <input
				type="hidden" name="type" value="selectByName" /> <input
				type="text" name="search" placeholder=" 录像名称" style="width:250px"
				class="input-text"> <input name="" id=""
				class="btn btn-success" type="submit">
		</form>
		<!-- TO-DO:实现批量删除 -->
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <!--<a href="javascript:;" onclick="data_del()" class="btn btn-danger radius"><i
            class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>--> <a
				class="btn btn-primary radius"
				onclick="teachingVideo_add('添加录像','/admin/video/teachingVideo-add.jsp')"
				href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加录像</a></span> <span
				class="r">共有数据：<strong>${sessionScope.resourceList.size()}</strong>
				条
			</span>
		</div>
		<div class="mt-20">
			<table id="coursewrae-table"
				...
				<tbody>
					<c:forEach items="${sessionScope.resourceList}" var="resource"
						varStatus="status">
						<tr class="text-c">
							<td><input type="checkbox" name="box" value="1"></td>
							<td>${resource.id}</td>
							<td style="text-align: left">
								<%--<img src="/images/teachResource/PPT.png">--%> <a
								href="${resource.resPath}" download="${resource.resName}">${resource.resName}</a>
							</td>
							...
							<td class="td-manage"><a style="text-decoration:none"
								...
								onClick="
                        <c:choose>
                        <c:when test="${resource.isCheck=='0'}">teachingVideo_start(this, ${resource.id})"
								title="发布"</c:when> <c:when test="${resource.isCheck=='1'}">teachingVideo_stop(this, ${resource.id})"  title="下架"</c:when>
									</c:choose> > <c:choose>
										<c:when test="${resource.isCheck == '0'}">
									...
									</c:choose> </a> <!--<a style="text-decoration:none" class="ml-5"
								onClick="teachingVideo_edit('课件编辑','teachingVideo-add.html')"
								href="javascript:;" title="编辑"> 
								<i class="Hui-iconfont">&#xe6df;</i> </a> --><a
								style="text-decoration:none" class="ml-5"
								onClick="teachingVideo_del(this, ${resource.id})"
